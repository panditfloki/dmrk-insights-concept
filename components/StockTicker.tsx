...
export default function StockTicker(){
  ...
  return <section className="market-ticker" aria-label="Listed Indian unicorn stocks"><div className="wrap market-ticker-label"><span><strong>Listed unicorns</strong> · BSE end-of-day prices</span><button type="button" onClick={()=>setHidden(v=>!v)} aria-expanded={!hidden} aria-controls={panelId}>{hidden?'Show':'Hide'}</button></div><div id={panelId} hidden={hidden}>{!hidden&&<><iframe title="BSE end-of-day stock prices from TradingView" src={source} sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" referrerPolicy="no-referrer" className="market-ticker-frame"/><div className="wrap market-ticker-credit"><a href="https://www.tradingview.com/markets/stocks-india/" target="_blank" rel="noopener noreferrer">Market data by TradingView</a><span> · If prices do not load, open market data.</span></div></>}</div></section>;
}
